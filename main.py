...
def get_sheet():
    try:
        ...
        if not creds_json:
            ...
            raise EnvironmentError("GOOGLE_CREDENTIALS not set")
        ...
        sheet_id = os.getenv("SHEET_ID", "108345771575623727353")
        ...
        logging.error(f"Invalid JSON in GOOGLE_CREDENTIALS: {e}")
        raise
...
TEXTS = {
    "ru": {
        ...
        "error_saving": "⚠️ Произошла ошибка при сохранении результатов. Пожалуйста, попробуйте позже."
    },
    "en": {
        ...
        "error_saving": "⚠️ An error occurred while saving results. Please try again later."
    }
}
...
def main():
    try:
        ...
        # Пытаемся подключиться к Google Sheets, но не падаем если не получилось
        try:
            get_sheet()
            logging.info("✅ Google Sheets connection successful")
        except Exception as e:
            logging.warning(f"⚠️ Google Sheets connection failed: {e}")
            logging.warning("Bot will continue without Google Sheets integration")
